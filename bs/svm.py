...
def random_pack_video(video, num_frms):
    ...
    if num == 1:
        ...
        pre = np.tile(np.zeros_like(video[0]), (thres, 1, 1))
        suf = np.tile(np.zeros_like(video[0]), (num_frms - len(video) - thres, 1, 1))
    ...
    for v_count, video in enumerate(lines):
        ...
        while ret:
            ...
            if np.max(opening) != np.min(opening):
                flow = cv2.calcOpticalFlowFarneback(prvs, next, flow=None, pyr_scale=0.5, levels=3,
                                                    winsize=15, iterations=3, poly_n=5, poly_sigma=1.2, flags=0)

                # calculate hof
                from hof import hof
                hof_image = hof(flow, orientations=30, pixels_per_cell=(20, 20))
                ...
                # x_feats.append(np.abs(delta_x.flatten()))
                # y_feats.append(delta_y.flatten())

                xy_feats.append(hof_image)
        ...
        xy_feats = np.vstack(xy_feats)
        xy_feats = np.mean(xy_feats, axis=0)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Get a svm classifier for fall action')
    parser.add_argument(
        '--data_path', default='/home/xiaoyu/Documents/action/datasets/ntu/test_videos')
    arg = parser.parse_args()

    prepare_feature(arg.data_path)
