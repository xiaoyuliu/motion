...
for video in os.listdir('../data'):
    ...
    while ret:
        ...
        if not ret:
            break
        down_frame2 = cv2.resize(frame2, (0, 0), fx=1.0 / SCALE, fy=1.0 / SCALE)
        next = cv2.cvtColor(down_frame2, cv2.COLOR_BGR2GRAY)
        ...
        V = -dy
        ax.clear()
        ax.quiver(X, Y, U, V, np.arctan2(V, U), scale=SCALE, linewidth=.004, width=0.001)
